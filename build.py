...
AUTHORS = [
    ...
    },
    {
        ...
        "links": [
            ("Scholar", "https://scholar.google.com/citations?user=2KruThAAAAAJ&hl=en"),
            ("LinkedIn", "https://www.linkedin.com/in/meysam-tavakoli-aa853228"),
        ],
    },
    {
        "initials": "SF",
...
def copy_author_photos() -> None:
    ...
    for a in AUTHORS:
        ...
        # Bake a circular alpha mask into the file itself. The site could round
        # these with CSS, but GitHub strips style attributes from README HTML,
        # so the only way to get circles in both places is an alpha channel.
        im = im.convert("RGBA")
        w = im.width
        ss = 4                                    # supersample for a clean edge
        mask = Image.new("L", (w * ss, w * ss), 0)
        ImageDraw.Draw(mask).ellipse((0, 0, w * ss - 1, w * ss - 1), fill=255)
        im.putalpha(mask.resize((w, w), Image.LANCZOS))
